...
class CRM_Redmine_Config {
  ...
  /**
   * CRM_Redmine_Config constructor.
   */
  function __construct() {

    $settings = civicrm_api3('Setting', 'Getsingle', array());
    $resourcesPath = $settings['extensionsDir'].'/net.trinfinity.redmine/resources/';
    try {
   ...
   * Singleton method
   *
   * @return CRM_Redmine_Config
   * @access public
   * @static
   */
  public static function singleton() {
    if (!self::$_singleton) {
      self::$_singleton = new CRM_Redmine_Config();
    }
  ...
  protected function setOptionGroups() {
    ...
    foreach ($optionGroups as $name => $optionGroupParams) {
      $optionGroup = new CRM_Redmine_OptionGroup();
  ...
  protected function setContactTypes() {
    ...
    foreach ($contactTypes as $name => $params) {
      $contactType = new CRM_Redmine_ContactType();
  ...
  protected function setCustomData() {
    ...
    foreach ($customData as $customGroupName => $customGroupData) {
      $customGroup = new CRM_Redmine_CustomGroup();
      ...
      foreach ($customGroupData['fields'] as $customFieldName => $customFieldData) {
        ...
        $customField = new CRM_Redmine_CustomField();
        $customField->create($customFieldData);
      }
      // remove custom fields that are still on install but no longer in config
      CRM_Redmine_CustomField::removeUnwantedCustomFields($created['id'], $customGroupData);
    }
  }
  ...
  protected function fixCustomFieldOptionGroups($customField, $customFieldOptionGroupName) {
    $optionGroup = CRM_Redmine_Utils::getOptionGroupWithName($customFieldOptionGroupName);
