 ...
 * @license AGPL-3.0
 */
class CRM_Redmine_CustomField {

  protected $_apiParams = array();

  /**
   * CRM_Redmine_CustomField constructor.
   */
  ...
  private function validateCreateParams($params) {
    if (!isset($params['name']) || empty($params['name']) || !isset($params['custom_group_id'])
      ...
      mandatory parameters and can not be empty in class CRM_Redmine_CustomField');
    }
  ...
  public function create($params) {
    ...
    if (!isset($this->_apiParams['label']) || empty($this->_apiParams['label'])) {
      $this->_apiParams['label'] = CRM_Redmine_Utils::buildLabelFromName($this->_apiParams['name']);
    }
    try {
