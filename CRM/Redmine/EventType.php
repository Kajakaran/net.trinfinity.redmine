 ...
 * @license AGPL-3.0
 */
class CRM_Redmine_EventType extends CRM_Redmine_OptionValue {
  /**
  ...
  protected function validateCreateParams($params) {
    if (!isset($params['name']) || empty($params['name'])) {
      throw new Exception('Missing mandatory param name in class CRM_Redmine_EventType');
    }
    $this->_apiParams = $params;
    try {
      $this->_apiParams['option_group_id'] = $this->getOptionGroupId();
    } catch (CiviCRM_API3_Exception $ex) {
      throw new Exception('Unable to find option group for event_type in CRM_Redmine_EventType, contact your system administrator');
    }
  }
